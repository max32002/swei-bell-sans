...
獅尾喇叭黑體基於[思源黑體](https://github.com/adobe-fonts/source-han-sans)的筆觸變喇叭口和拔腳改造，更加簡明現代化的字體。支援简体中文、繁體中文、韓文與日文；可以免費商用，歡迎大家自由應用、自由優化、自由改作！

獅尾喇叭黑體的特色是：筆畫的起頭與收尾較寬，有喇叭造型。「喇叭口」在鉛字印刷時代有讓印刷字安定清晰的用途，目前電腦螢幕顯示與高解析度輸出的數位時代，喇叭口變成字型的風格的一部份。

「獅尾喇叭黑體」與「獅尾喇腿黑體」差別在有沒有拔腳，「拔腳」就是會有較少的筆畫，例如移除「口」字的左右下角會多出的短線條。有拔腳的字會較圓，但是會造成有些字因直接去掉腿後左右不平衡、左右不對襯的「懸空」問題。改使用「腿」系列字體就不會有這個問題。
...
附註：由於目前喇叭效果設定較小（向外擴大筆寬的12%），所以當文字尺寸較小時，會看不出差異。
...
### 「CJK TC」和「CJK SC」的比較：
![TC和SC比較](https://github.com/max32002/swei-bell-sans/raw/master/preview/compare_tc_sc.png)
...
## 更新日誌
[點擊此處](https://github.com/max32002/swei-bell-sans/blob/master/change_log.md) 查看更新記錄。
...
```
@font-face {
  font-family: SweiBellSansCJKtc-Regular;
  src: url(https://cdn.jsdelivr.net/gh/max32002/swei-bell-sans@2.0/WebFont/CJK%20TC/SweiBellSansCJKtc-Regular.woff2) format("woff2")
  , url(https://cdn.jsdelivr.net/gh/max32002/swei-bell-sans@2.0/WebFont/CJK%20TC/SweiBellSansCJKtc-Regular.woff) format("woff");
}
```
